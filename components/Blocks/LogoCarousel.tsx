...
const SimpleIcon = ({ iconSlug, size = 24 }: { iconSlug: string; size?: number }) => {
  const simpleIcon = simpleIconMap[iconSlug as keyof typeof simpleIconMap]
  ...
  if (!iconPath) {
    ...
    return (
      <span
        className='flex shrink-0 items-center justify-center rounded-md bg-foreground text-[9px] font-bold text-background'
        style={{ width: size, height: size }}
      >
        {iconLabels[iconSlug] ?? iconSlug.slice(0, 2).toUpperCase()}
      </span>
    )
  }
...
// Frameworks and tools used across modern web projects.
const techCompanies = [
